...
const TodoListIndex: React.FC = () => {
  ...
  const [list, setList] = useState<IListItem[]>(
    () => JSON.parse(localStorage.getItem('list') || '') || InitTodoListData
  )
  ...
  const onOpenInsertSection = () => {
    setOsTilte('Todo Add')
    setOsBody(
      <TodoInsert onTodoInsert={onTodoInsert} lastItemId={lastItemId} />
    )
    setOpen(true)
  }
  ...
  const onOpenDetailSection = (todo: IListItem) => {
    ...
    setOsBody(
      <TodoDetail todo={todo} onOpenRewriteSection={onOpenRewriteSection} />
    )
    setOpen(true)
  }
  ...
  const openSection = open ? <OpenSection {...openSectionViewProps} /> : ''
